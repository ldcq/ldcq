...
def visualize_states(state_0, states, best_state):
    ...
    plt.scatter(6.934579, 8.953581, color='blue')
    plt.xlim(0, 8)
    plt.ylim(0, 10)
...
def prior_policy(
    ...
    state = state_0.clone()
    latent, latent_prior_std = skill_model.prior(state)
...
def eval_func(diffusion_model,
    ...
    with torch.no_grad():
        ...
        for eval_step in range(num_evals):
            ...
            success_eval = [False] * num_parallel_envs

            for env_idx in range(len(envs)):
                state_0[env_idx] = torch.from_numpy(envs[env_idx].reset())
                # envs[env_idx].set_state(np.array([5, 7.5]), np.array([0, 0]))
                goal_state[env_idx][0] = 6.934579 
                goal_state[env_idx][1] = 8.953581
            ...
            success_evals += sum(success_eval)
            ...
            print(f'Total D4RL score: {envs[0].get_normalized_score(scores / total_runs) * 100}')
